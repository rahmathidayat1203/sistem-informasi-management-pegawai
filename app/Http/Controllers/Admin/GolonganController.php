...
class GolonganController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $golongans = Golongan::select('*');

            return DataTables::of($golongans)
                ->addColumn('action', function ($row) {
                    $showUrl = route('admin.golongan.show', $row->id);
                    $editUrl = route('admin.golongan.edit', $row->id);
                    $deleteUrl = route('admin.golongan.destroy', $row->id);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.golongan.index')->with('success', 'Golongan berhasil ditambahkan.');
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update(Request $request, string $id)
    {
        ...
        $request->validate([
            'nama' => 'required|string|max:255|unique:golongan,nama,' . $id,
            'deskripsi' => 'nullable|string',
        ]);
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.golongan.index')->with('success', 'Golongan berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollback();
            return redirect()->back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage());
        }
